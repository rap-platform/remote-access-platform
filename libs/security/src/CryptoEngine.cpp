...
void chacha20Block(uint32_t out[16],
...
void chacha20Xor(uint8_t* data,
    ...
    for (int i = 0; i < 8; ++i)
        key32[i] = reinterpret_cast<const uint32_t*>(key)[i];
    for (int i = 0; i < 3; ++i)
        nonce32[i] = reinterpret_cast<const uint32_t*>(nonce)[i];
...
CryptoEngine::decryptPayload(const std::vector<uint8_t>& ciphertext,
                             const std::vector<uint8_t>& key,
                             const std::vector<uint8_t>& nonce) {
