...
RAP_EXPORT int32_t rap_mobile_get_p2p_id(char* out_buf, size_t buf_len) {
    if (!out_buf || buf_len < 12) return -1;

    QByteArray hardwareData = (QHostInfo::localHostName() + QSysInfo::machineUniqueId() + QSysInfo::bootUniqueId()).toUtf8();
...
RAP_EXPORT int32_t rap_mobile_encode_packet(
    ...
    if (!out_buf || !written_len) return -1;
    ...
    auto encoded = rap::protocol::ProtocolCodec::encode(
        static_cast<rap::protocol::PayloadType>(payload_type),
        sequence_num,
        timestamp_ms,
        payloadVec
    );

    if (encoded.size() > max_out_len) return -2;
...
RAP_EXPORT int32_t rap_mobile_decode_packet(
    const uint8_t* in_buf,
    size_t in_len,
    rap_c_packet_t* out_packet
) {
    if (!in_buf || !out_packet || in_len < 28) return -1;
