...
int main(int argc, char* argv[]) {
    ...
    qInfo().noquote() << QString("[Agent Security] Host Dynamic One-Time Passcode (OTP): \"%1\"")
                             .arg(formattedOtp);
    ...
    if (inputBackend) {
        ...
        qInfo() << "[Agent] Remote Input Injection initialized:"
                << QString::fromStdString(inputBackend->backendName());
    }

    qInfo() << "[Agent] Screen capture initialized:"
            << QString::fromStdString(captureBackend->backendName());
    ...
    if (!server.listen(QHostAddress::Any, 18443)) {
        qCritical() << "[Agent] Failed to bind local TCP server on port 18443:"
                    << server.errorString();
        return 1;
    }
    ...
    QObject::connect(
        ...
        [&server, &clients, &inputBackend, sessionKey, dynamicOtp]() {
            while (server.hasPendingConnections()) {
                ...
                bool isLoopback = clientSocket->peerAddress().isLoopback() ||
                                  peerStr.contains("127.") || peerStr.contains("::1") ||
                                  peerStr.contains("localhost") || peerStr.isEmpty();
                clientSocket->setProperty("isLoopback", isLoopback);
                if (isLoopback) {
                    qInfo()
                        << "[Agent Input Guard] Client is local loopback (" << peerStr
                        << "). Bypassing X11 local input injection to prevent terminal feedback.";
                ...
                QObject::connect(
                    ...
                    [clientSocket, &inputBackend, sessionKey, dynamicOtp]() {
                        ...
                        while (buffer.size() >= 28) {
                            ...
                            rap::agent::handleClientPacket(clientSocket,
                                                           packet,
                                                           inputBackend,
                                                           sessionKey,
                                                           dynamicOtp);
    ...
    captureBackend->startCapture([&app, &clients, sessionKey](
                                     const rap::capture::FrameData& frame) {
        if (clients.isEmpty())
            return;

        qint64 t0 = QDateTime::currentMSecsSinceEpoch();
        uint32_t w = frame.width, h = frame.height;
        QByteArray rawPixels(reinterpret_cast<const char*>(frame.pixelData.data()),
                             static_cast<qsizetype>(frame.pixelData.size()));
        ...
        qInfo().noquote()
            << QString("[Agent Latency Audit] Frame #%1 | Encrypt & Auth (%2 KB -> %3 KB): %4 ms")
